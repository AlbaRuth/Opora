...
from integrations.telegram.prescreening import (
    ...
    build_styles_keyboard,  # NEW: renamed from build_styles_keyboard
    THERAPIST_STYLES,  # NEW: renamed from THERAPIST_STYLES
...
class TestPrescreeningState:
    ...
    def test_default_state(self):
        ...
        assert state.selected_styles == []

    def test_custom_state(self):
        """Test prescreening state with custom values."""
        state = PrescreeningState(
            step="awaiting_styles_selection",
            therapist_name="Доктор",
            therapist_gender="male",
            patient_name="Иван",
            patient_age=30,
            selected_styles=["friendly", "soft"],
        )

        assert state.step == "awaiting_styles_selection"
        ...
        assert state.selected_styles == ["friendly", "soft"]
...
class TestKeyboardBuilders:
    ...
    def test_build_styles_keyboard_empty(self):
        """Test styles keyboard without selections."""
        keyboard = build_styles_keyboard([])

        # Should have one button per style, no Done button
        assert len(keyboard.inline_keyboard) == len(THERAPIST_STYLES)

        # Check first style button
        first_button = keyboard.inline_keyboard[0][0]
        assert "⬜ " in first_button.text or "✅ " in first_button.text
        assert first_button.callback_data.startswith("prescreen:style:")

    def test_build_styles_keyboard_with_selections(self):
        """Test styles keyboard with some selections."""
        selected = ["friendly", "soft"]
        keyboard = build_styles_keyboard(selected)

        # Should have style buttons + Done button
        assert len(keyboard.inline_keyboard) == len(THERAPIST_STYLES) + 1

        # Check Done button is last
        last_row = keyboard.inline_keyboard[-1]
        assert len(last_row) == 1
        assert "Готово" in last_row[0].text
        assert last_row[0].callback_data == "prescreen:styles_done"
...
class TestPrescreeningConstants:
    ...
    def test_therapist_styles_defined(self):
        """Test that therapist styles are defined."""
        assert len(THERAPIST_STYLES) >= 4

        # Check format
        for style_id, style_label in THERAPIST_STYLES:
            assert isinstance(style_id, str)
            assert isinstance(style_label, str)
            assert len(style_id) > 0
            assert len(style_label) > 0
    ...
    def test_style_ids_unique(self):
        """Test that style IDs are unique."""
        style_ids = [sid for sid, _ in THERAPIST_STYLES]
